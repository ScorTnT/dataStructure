...
void complementSet();           //complement Set.
void RcomSet();             //relative complement
int main(){
    int command=6;
    printf("what do you want?\n(letSet:0, delSet:1, insElement:2, delElement:3, AndGate:4, OrGate:5, ComplementGate:6, RelativeComplement:7): ");
    scanf("%d",&command);
    printf("\n");
    if(command>7) {
        printf("wrong order.");
        ...
        case 6://complement gate
            ...
            main();
        break;
        case 7://relative complement gate
            RcomSet();
            main();
        break;
        default:
...
void RcomSet(){
    int orind1=0,orind2=0;
    set *reComSet=(set *)malloc(sizeof(set)),*newCom,*tmp,*r;      //select insert 
    reComSet->element=7777;
    reComSet->nextptr=NULL;
    set *check,*s,*HEAD,*t;
    HEAD=reComSet;
    sets *p,*q;
    q=&setsHead;
    ...
    while (empty(s)!=0){
        while(empty(t)!=0){
            if(s->element==t->element) orind1++;
            t=t->nextptr;
        }
        if(orind1==0){
            newCom=(set *)malloc(sizeof(set));
            newCom->element=s->element;
            if(empty(reComSet)==0){
                newCom->nextptr=reComSet;
                HEAD=newCom;
                reComSet=HEAD;
            }else{
                while (empty(reComSet->nextptr)){
                    reComSet=reComSet->nextptr;
                }
                newCom->nextptr=reComSet->nextptr;
                reComSet->nextptr=newCom;
            }
        }
        orind1=0;
        s=s->nextptr;
